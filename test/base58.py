...
def public_key_to_bc(public_key, version="\x00", typeret=0):
  if not have_crypto or public_key is None:
    return ''
  h160 = hash_160(public_key)
  return h160.encode('hex'), hash_160_to_bc_address(h160, version=version), typeret

def hash_160_to_bc(h160, version="\x00",typeret=0):
  if not have_crypto:
    return ''
  vh160 = version+h160
  h3=hashlib.sha256(hashlib.sha256(vh160).digest()).digest()
  addr=vh160+h3[0:4]
  return h160.encode('hex'), b58encode(addr), typeret
...
if __name__ == '__main__':
    ...
    x = '00010966776006953D5567439E5E39F86A0D273BEED61967F6'.decode('hex_codec')
    encoded = b58encode(x)
    #print encoded, '19TbMSWwHvnxAKy12iNm3KdbGfzfaMFViT'
    print encoded, '16UwLL9Risc3QfPqBUvKofHmBQ7wMtjvM'
