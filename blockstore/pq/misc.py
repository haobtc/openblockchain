...
def itercol(conn, col, key, n, batch=1):
    ...
    while total <= n:
        ...
        if v:
            objid = v['objid']
            t = v.get('times', 0)
            objs = list(col.find({'_id': {'$gt': objid}}).sort(
                [('_id', 1)]).limit(batch))
        else:
            objs = list(col.find().sort([('_id', 1)]).limit(batch))
        if len(objs) == 0:
            break
        total += len(objs)
        t += len(objs)
...
def return_borrowed_peers(conn):
    ...
    conn.peerpool.update(
        {'borrowed': 1,
         'lastBorrowed': {'$lt': now_time - 300}}, {'$set': {'borrowed': 0}},
        multi=True)
...
def push_peers(conn, peers):
    now_time = int(time.time())
    for peer in peers:
        conn.peerpool.update({'host': peer.host,
                              'port': peer.port},
    ...
    for p in arr:
        ...
        conn.peerpool.update(
            {'host': peer.host,
             'port': peer.port},
            {'$set': {
                'borrowed': 1, 'lastBorrowed': now_time
            }})
    return peers
