...
def deserialize_CAddress(d):
    return d['ip'] + ":" + str(d['port']) + " (lastseen: %s)" % (
        time.ctime(d['nTime']), )
...
def deserialize_TxIn(d, transaction_index=None, owner_keys=None):
    if d['prevout_hash'] == "\x00" * 32:
        ...
        result += " prev(" + long_hex(d['prevout_hash'][::-1]) + ":" + str(
            d['prevout_n']) + ")"
    else:
        result = "TxIn: prev(" + long_hex(d['prevout_hash'][::-1]) + ":" + str(
            d['prevout_n']) + ")"
...
def deserialize_Transaction(d,
    ...
    if print_raw_tx == True:
        result += "Transaction hex value: " + d['__data__'].encode(
            'hex') + "\n"
...
def deserialize_Block(d, print_raw_tx=False):
    ...
    result += "\nhashMerkleRoot: 0x" + d['hashMerkleRoot'][::-1].encode(
        'hex_codec')
