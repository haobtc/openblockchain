...
BEGIN
    ...
    tx_count2 = (select coalesce(count(1),0) from tx where id<=max_id.tx_id and id>const_stat.max_tx_id) -  (select coalesce(count(1),0) from utx where id<=max_id.tx_id and id>const_stat.max_tx_id);
